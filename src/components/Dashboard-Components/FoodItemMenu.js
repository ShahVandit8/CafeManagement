        ...
        <tbody>
          {
            foodItem.map(foodItem => (
                <tr key={foodItem._id}>
                    ...
                    <td>
                    <Link to={`/dashboard/editfooditem/${foodItem._id}`} className="btn btn-primary" style={{marginRight: 5}}>Edit</Link>
                    <button type="button" className="btn btn-danger" onClick={() => deleteFoodItem(foodItem._id)}>Delete</button>  
                    </td>
                </tr>
            ))
          }
