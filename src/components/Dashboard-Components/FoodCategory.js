        ...
        <tbody>
          {
            category.map(category => (
                <tr key={category._id}>
                    ...
                    <td>
                    <Link to={`/dashboard/editfoodcategory/${category._id}`} className="btn btn-primary" style={{marginRight: 5}}>Edit</Link>
                    <button type="button" className="btn btn-danger" onClick={() => deleteFoodCategory(category._id)}>Delete</button>  
                    </td>
                </tr>
            ))
          }
